...
## 常见问题
### 问题1： 验证码登录用户，再通过导入，数据库出现2条记录？
答案： 原因是通过验证码登录，会注册一个用户，用户ID会随机生成。当导入时，excel表哥中的userId列填写了用户ID，此ID与生成的不一致，则导入会创建另外一个用户。这就导致数据库中出现有相同手机号码的2个用户。下述任一方法都可以避免此问题：
1. 导入时不填写userId，用系统默认。
2. 关闭应用服务注册功能，只能组织通讯录服务添加。
3. 导入时先找到已经创建的用户的用户ID，填上正确ID。
